...
7. Use `git reset` + `--hard` + `HEAD^` to undo the last commit. Notice that `HEAD^` means the last commit, `HEAD^^` means the commit before the last commit, and so on. Or, use `git reset` + `--soft` + `HEAD^` to undo the last commit and keep the changes in the staging area. Or, use `git reset` + `--mixed` + `HEAD^` to undo the last commit and remove the changes from the staging area.
8. 